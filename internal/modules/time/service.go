...
func (s *Service) StartTimer(userID, projectID, description string) (*types.TimeEntry, error) {
	...
	// Save to Badger
	err = s.db.TimeEntries().Save(entry)
	if err != nil {
		return nil, fmt.Errorf("failed to save time entry: %w", err)
	}
	
	// Publish event
	event := types.NewEvent("session_started", "time_service", map[string]interface{}{
...
func (s *Service) StopTimer(userID string) (*types.TimeEntry, error) {
	entry, err := s.db.TimeEntries().GetActiveTimer(userID)
	if err != nil {
	...
	// Publish event
	event := types.NewEvent("session_stopped", "time_service", map[string]interface{}{
		"time_entry_id": entry.ID,
		"user_id":       userID,
...
func (s *Service) PauseTimer(userID string) (*types.TimeEntry, error) {
	...
	// Publish event
	event := types.NewEvent("session_paused", "time_service", map[string]interface{}{
		"time_entry_id": entry.ID,
		"user_id":       userID,
...
func (s *Service) ResumeTimer(userID string) (*types.TimeEntry, error) {
	...
	// Publish event
	event := types.NewEvent("session_resumed", "time_service", map[string]interface{}{
		"time_entry_id": entry.ID,
		"user_id":       userID,
...
func (s *Service) handleProjectStarted(event *types.Event) error {
	...
	suggestionEvent := types.NewEvent("timer_suggestion", "time_service", map[string]interface{}{
		"user_id":    userID,
		"project_id": projectID,
...
func (s *Service) handleInvoiceGenerated(event *types.Event) error {
	...
	billedEvent := types.NewEvent("time_entries_billed", "time_service", map[string]interface{}{
...
func (s *Service) UpdateTimeEntry(userID, timeEntryID, description string, tags []string) error {
	entry, err := s.db.TimeEntries().Get(timeEntryID)
	if err != nil {
	...
	event := types.NewEvent("session_updated", "time_service", map[string]interface{}{
